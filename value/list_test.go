...
func TestList(t *testing.T) {
	runTest(t, []testType{
		...
		{exp: "[1,2,3].indexWhere(e->e=4)", res: Int(-1)},
		{exp: "[1,2,3].indexWhere(e->e=7)", res: Int(-1)},
		...
		{exp: "[1,2,3].accept(i->i>=1)", res: NewList(Int(1), Int(2), Int(3))},
		{exp: "[1,2,3,4,5].sum()", res: Int(15)},
		{exp: "[1,2,3,4,5].reduce((a,b)->a+b)", res: Int(15)},
		{exp: "[1,2,3,4,5].reduce((a,b)->max(a,b))", res: Int(5)},
		...
		{exp: "[1,5,3,2,4].max()", res: Int(5)},
		{exp: "try [].min() catch \"error\"", res: String("error")},
		{exp: "try [].max() catch \"error\"", res: String("error")},
...
func TestParallel(t *testing.T) {
	fu, _, err := New().Generate(parallel)
	assert.NoError(t, err)
...
func TestParallelError(t *testing.T) {
	fu, _, err := New().AddStaticFunction("error", toLargeErrorFunc(500)).Generate(parallelError)
	assert.NoError(t, err)
...
func TestParallelError2(t *testing.T) {
	fu, _, err := New().AddStaticFunction("error", toLargeErrorFunc(500)).Generate(parallelError2)
	assert.NoError(t, err)
	_, err = fu(funcGen.NewEmptyStack[Value]())
	assert.Error(t, err)
	assert.True(t, strings.Contains(err.Error(), "toLarge"))
