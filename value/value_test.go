...
func BenchmarkRegulaFalsi(b *testing.B) {
	f, _ := New(nil).Generate(regulaFalsi, "a")
...
func BenchmarkCall(b *testing.B) {
	f, _ := New(nil).Generate("x+(2*y/x)", "x", "y")
...
func BenchmarkFunc(b *testing.B) {
	f, _ := New(nil).Generate("func f(x) x*x;f(a)+f(2*a)", "a")
...
func BenchmarkFunc2(b *testing.B) {
	f, _ := New(nil).Generate("let c=1.5;func mul(x) y->y*x*c;mul(b)(a)", "a", "b")
...
func BenchmarkList(b *testing.B) {
	f, err := New(nil).Generate("l.map(e->e*e).map(e->e/100)", "l")
	if err != nil {
		fmt.Println(err)
	}
