...
let lp = createLowPass("f" , p->p.t, p->p.s, 1);

data.iirApply(lp).size()
`

func BenchmarkFilter1(b *testing.B) {
	f, err := SetUpParser(New()).Generate(filter1)
	if err != nil {
		fmt.Println(err)
	}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		f(funcGen.NewEmptyStack[Value]())
	}
}
...
func clp(name, getT, getX, tau)
	{initial: e->e.put(name, getX(e)),
     filter:(e0,e1,l)->
		 ...
		 let dt = t1 - t0;
		 let a = exp(-dt / tau);
         e.put(name, y*a + x*(1-a))};

let lp = clp("f" , p->p.t, p->p.s, 1);

data.iirApply(lp).size()
`

func BenchmarkFilter2(b *testing.B) {
	f, err := SetUpParser(New()).Generate(filter2)
	if err != nil {
		fmt.Println(err)
	}
	b.ResetTimer()
