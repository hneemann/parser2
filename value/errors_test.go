...
func TestErrors(t *testing.T) {
	tests := []struct {
		...
		{"list(0).single()", "not a single"},
		{"list(2).single()", "more than one"},
		...
		{"list(1000000000).multiUse({a:l->l.mapReduce(0,(s,i)->s+error(i)), b:l->l.mapReduce((s,i)->s+i)})", "wrong number of arguments"},
