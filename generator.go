...
func (g *FunctionGenerator[V]) Generate(exp string) (func(v Variables[V]) (V, error), error) {
	ast, err := g.CreateAst(exp)
	if err != nil {
		return nil, err
	}

	f, err := g.GenerateFunc(ast)
	if err != nil {
		return nil, err
	}

	return func(v Variables[V]) (val V, err error) {
		defer func() {
			rec := recover()
			if rec != nil {
				var zero V
